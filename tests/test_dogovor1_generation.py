"""Integration test: parse dogovor1.docx and generate KS-2/KS-3, compare with reference files."""
...
from app.models import ExtractionResult, RowGroupingMode
...
CONTRACT_PATH = INCOMING / "dogovor1.docx"
KS2_REFERENCE = INCOMING / "ks2_1verno.xlsx"
KS3_REFERENCE = INCOMING / "ks3_1verno.xlsx"
...
def _prepare_output_dir() -> Path:
    out = PROJECT_ROOT / "output" / "test_dogovor1"
    if out.exists():
        shutil.rmtree(out)
...
def _build_controller():
    """Build controller with real parser stubs."""
    config = AppConfig(
        paths=PathsConfig(
...
def _full_extraction(contract_path: Path, ctrl: dict):
    ...
    return contract, cls, result


class TestDogovor1ParsingAndGeneration:
    """Parse dogovor1.docx and generate KS-2/KS-3, then compare with reference files."""
    ...
    def setup(self):
        ...
        self.contract, self.cls, self.result = _full_extraction(CONTRACT_PATH, self.ctrl)
    ...
    def test_header_extraction(self):
        ...
        assert doc.contract_number == "14/03-07"
    ...
    def test_totals_extracted(self):
        ...
        assert t.vat_rate > 0, "VAT rate not extracted"

    def test_ks2_generation(self):
        """Test KS-2 generation from dogovor1.docx — structural validation."""
        ...
        self._validate_ks2_structure(generated)

    def test_ks3_generation(self):
        """Test KS-3 generation from dogovor1.docx — structural validation."""
        ...
        self._validate_ks3_structure(generated)

    def _validate_ks2_structure(self, path: Path):
        """Validate KS-2 structure: headers, data rows, totals, signatures."""
        wb = load_workbook(str(path), data_only=False)
        ws = wb.active

        # Check header fields
        assert ws["L10"].value is not None and "14/03-07" in str(ws["L10"].value), "Contract number missing"
        assert ws["L11"].value == "03", f"Contract day: {ws['L11'].value}"
        assert ws["N11"].value == "07", f"Contract month: {ws['N11'].value}"
        assert ws["O11"].value == "2025", f"Contract year: {ws['O11'].value}"

        # Check data rows exist (at least 5)
        data_rows = 0
        for row in range(25, 45):
            c = ws.cell(row, 3).value
            if c and "Итого" not in str(c) and "НДС" not in str(c):
                data_rows += 1
        assert data_rows >= 5, f"Only {data_rows} data rows found"

        # Check totals (New templates: Total in I27/IP or I28/ORG)
        found_total = False
        for row in [27, 28]:
            i_val = ws.cell(row, 9).value # Column I
            if i_val is not None and float(str(i_val).replace(" ", "").replace("\xa0", "")) > 0:
                found_total = True
                break
        assert found_total, "Total amount in column I missing"

        # Check signatures dynamically — find "Сдал" and "Принял" markers
        executor_row = None
        customer_row = None
        for row in range(28, ws.max_row + 1):
            a_val = ws.cell(row, 1).value
            if isinstance(a_val, str):
                if "сдал" in a_val.lower() and executor_row is None:
                    executor_row = row
                elif "принял" in a_val.lower() and customer_row is None:
                    customer_row = row

        assert executor_row is not None, "Executor signature block ('Сдал') not found"
        assert customer_row is not None, "Customer signature block ('Принял') not found"

        g_exec = ws.cell(executor_row, 7).value
        g_cust = ws.cell(customer_row, 7).value
        assert g_exec is not None and str(g_exec).strip(), f"Executor signature (G{executor_row}) missing"
        assert g_cust is not None and str(g_cust).strip(), f"Customer signature (G{customer_row}) missing"

    def _validate_ks3_structure(self, path: Path):
        """Validate KS-3 structure: headers, totals, signatures."""
        wb = load_workbook(str(path), data_only=False)
        ws = wb.active

        # Check headers
        assert ws["O12"].value is not None and "14/03-07" in str(ws["O12"].value), "Contract number missing"
        assert ws["O13"].value == "03", f"Contract day: {ws['O13'].value}"
        assert ws["Q13"].value == "07", f"Contract month: {ws['Q13'].value}"
        assert ws["R13"].value == "2025", f"Contract year: {ws['R13'].value}"

        # Check totals in column N (New templates: N30, N31, N32 for ORG)
        # dogovor1.docx is ORG-ORG, so totals start at N30
        assert ws["N30"].value is not None, "N30 (Итого без НДС) missing"
        assert ws["N31"].value is not None, "N31 (НДС) missing"
        assert ws["N32"].value is not None, "N32 (Всего с НДС) missing"
